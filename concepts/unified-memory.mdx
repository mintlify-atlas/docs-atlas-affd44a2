...
description: Learn how MLX takes advantage of Apple Silicon's unified memory architecture
---
...
In the above, both the CPU and the GPU will perform the same add operation. The operations can (and likely will) be run in parallel since there are no dependencies between them.

<Note>
See the [Streams documentation](/guides/streams) for more information on the semantics of streams in MLX.
</Note>
